...
def main():
    ...
    parser.add_argument('-or', '--outputdir-result', dest='outputdir_result', type=str,default=f'Result/all_indicator/result.xlsx')
    parser.add_argument('-of', '--outputdir-file', dest='outputdir_file', type=str,default=f'Result/all_file/result/')
    parser.add_argument('-dw', '--dir-weight', dest='dir_weight', type=str, default=f'Result/Weight/result/')
    ...
    with open(data_index_file, 'rb') as f:
        pdb_tag_train = pickle.load(f)
    pdb_tag_train = np.array(pdb_tag_train)
    all_pdb_tag = pdb_tag_train
    with open(data_label_index_file, 'rb') as f:
        pdb_label_train = pickle.load(f)
    pdb_label_train = np.array(pdb_label_train)
    with open('Data/MPD48/data_index/MPD48.pkl', 'rb') as f:
        pdb_tag_test = pickle.load(f)


    x = 0
    for t in range(len(pdb_label_train)):
        if pdb_label_train[t] == 0:
            x += 1
    alpha = x/(len(pdb_tag_train))
    xishu = (len(pdb_tag_train)-x)/x
    criterion = nn.MSELoss()
    fold_num = 0
    data_test = dataset.mydataset(pdb_tag_test, 'Data/MPD48')
    ...
    for train_inx, valid_inx in cv.split(pdb_tag_train, pdb_label_train):
        ...
        for epoch in range(epochs):
            ...
            print(f"第{epoch}次epoch下验证集的val_loss为：{val_loss}")
            print(f"第{epoch}次epoch下验证集的PCC为{pcc:.4f}，RMSE为{rmse}，MAE为{mae}")
        ...
        test_predict = []
        test_y = []
        torch.save(best_model_wts, dir_weight + '{}_valid_test.pth'.format(fold_num))
